...
install_success(){
	echo "----------------------------"
	#推送数据到普罗米修斯以自动注册
	curl -u "${API_USER_PASS}" -X POST -d "instance=${INSTANCE}" https://prometheus.rss.ink/api/v1/push_data
...
uninstall(){
	...
	rm -rf ${INSTALL_PATH}
	#请求API移除服务端数据
	curl -u "${API_USER_PASS}" https://prometheus.rss.ink/api/v1/push_data?type=delete
	echo "----------------------------"
